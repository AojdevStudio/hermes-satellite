...
**Host-side viewer:** the `hst` CLI (`scripts/hst.ts`) reads all of this from the bridge DB with read-only semantics: it opens SQLite read-write for WAL compatibility, then sets `PRAGMA query_only=ON`; open failures exit as a one-line `hst: cannot open bridge db` error. `hst costs` shows per-task snapshots with the caller and honest cost rendering — `$0` on a subscription-billed session prints as `$0 (subscription)`, any other `$0` prints as `unknown`, never free; `hst task <id>` applies the same rule to per-loop cost lines. `hst tasks` summarizes prompts as short LLM-generated gists (Groq via `GROQ_API_KEY`, else OpenRouter `:free` via `OPENROUTER_API_KEY`, overridden by `HST_GIST_MODEL`; cached in `~/.hermes/cache/hst_summaries.json`; silent fallback to the raw prompt on missing keys, timeouts, network failures, or OpenRouter rate limits). `--json` on `tasks`/`task`/`events`/`costs` never networks.
